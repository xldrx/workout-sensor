...
if ~regress
  ...
  assert2(approxeq(muY, mean(Y')))
  assert2(approxeq(SigmaY, cov(Y') + 0.01*eye(ny)))
else
...
if nx==1 & ny==1 & regress
  ...
  assert2(approxeq(muY, P(2)))
  assert2(approxeq(weightsY, P(1)))
end
...
if 0 
  ...
  assert2(approxeq(b(1),m))
  assert2(approxeq(b(2), w(1)))
  assert2(approxeq(b(3), w(2)))
end
