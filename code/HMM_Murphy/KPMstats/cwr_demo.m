...
% Check equal
cwr_pc = cwr.priorC';
assert2(approxeq(cwr_pc, pc))
cwr_mux = cwr.muX;
assert2(approxeq(mux, cwr_mux))
cwr_SigmaX = squeeze(cwr.SigmaX)';
assert2(approxeq(varx, cwr_SigmaX))
cwr_muy = cwr.muY;
assert2(approxeq(muy, cwr_muy))
cwr_SigmaY = squeeze(cwr.SigmaY)';
assert2(approxeq(vary, cwr_SigmaY))
...
% Check equal
assert2(approxeq(yplot(:)', cwr_mu(:)'))
assert2(approxeq(ystdplot, cwr_ystd))
assert2(approxeq(pplot ./ repmat(sum(pplot,2), 1, nclusters),post') )
